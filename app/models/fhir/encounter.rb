    ...
    def self.transform_json(json_hash, target = Encounter.new)
      ...
      result['statusHistory'] = json_hash['statusHistory'].map { |var| 
        unless var['resourceType'].nil?
          ...
          EncounterStatusHistory.transform_json(var) 
        end
      } unless json_hash['statusHistory'].nil?
      result['class'] = Coding.transform_json(json_hash['class']) unless json_hash['class'].nil?
      result['classHistory'] = json_hash['classHistory'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          EncounterClassHistory.transform_json(var) 
        end
      } unless json_hash['classHistory'].nil?
      ...
      result['episodeOfCare'] = json_hash['episodeOfCare'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          Reference.transform_json(var) 
        end
      } unless json_hash['episodeOfCare'].nil?
      ...
      result['participant'] = json_hash['participant'].map { |var| 
        unless var['resourceType'].nil?
          ...
          EncounterParticipant.transform_json(var) 
        end
      } unless json_hash['participant'].nil?
      result['appointment'] = json_hash['appointment'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          Reference.transform_json(var) 
        end
      } unless json_hash['appointment'].nil?
      ...
      result['diagnosis'] = json_hash['diagnosis'].map { |var| 
        unless var['resourceType'].nil?
          ...
          EncounterDiagnosis.transform_json(var) 
        end
      ...
      result['location'] = json_hash['location'].map { |var| 
        unless var['resourceType'].nil?
          ...
          EncounterLocation.transform_json(var) 
        end
