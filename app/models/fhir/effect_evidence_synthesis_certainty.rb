module FHIR
  ...
  class EffectEvidenceSynthesisCertainty < BackboneElement
    ...
    def self.transform_json(json_hash, target = EffectEvidenceSynthesisCertainty.new)
      ...
      result['certaintySubcomponent'] = json_hash['certaintySubcomponent'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          EffectEvidenceSynthesisCertaintyCertaintySubcomponent.transform_json(var) 
        end
      } unless json_hash['certaintySubcomponent'].nil?
