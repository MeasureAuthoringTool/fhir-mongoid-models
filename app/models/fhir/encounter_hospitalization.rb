module FHIR
  ...
  class EncounterHospitalization < BackboneElement
    ...
    def self.transform_json(json_hash, target = EncounterHospitalization.new)
      ...
      result['dietPreference'] = json_hash['dietPreference'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          CodeableConcept.transform_json(var) 
        end
      } unless json_hash['dietPreference'].nil?
      result['specialCourtesy'] = json_hash['specialCourtesy'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          CodeableConcept.transform_json(var) 
        end
      } unless json_hash['specialCourtesy'].nil?
      result['specialArrangement'] = json_hash['specialArrangement'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          CodeableConcept.transform_json(var) 
        end
      } unless json_hash['specialArrangement'].nil?
