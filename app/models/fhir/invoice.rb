    ...
    def self.transform_json(json_hash, target = Invoice.new)
      ...
      result['participant'] = json_hash['participant'].map { |var| 
        unless var['resourceType'].nil?
          ...
          InvoiceParticipant.transform_json(var) 
        end
      ...
      result['lineItem'] = json_hash['lineItem'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          InvoiceLineItem.transform_json(var) 
        end
      } unless json_hash['lineItem'].nil?
      result['totalPriceComponent'] = json_hash['totalPriceComponent'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          InvoiceLineItemPriceComponent.transform_json(var) 
        end
      } unless json_hash['totalPriceComponent'].nil?
