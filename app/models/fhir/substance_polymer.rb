module FHIR
  ...
  class SubstancePolymer < DomainResource
    ...
    def self.transform_json(json_hash, target = SubstancePolymer.new)
      ...
      result['copolymerConnectivity'] = json_hash['copolymerConnectivity'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          CodeableConcept.transform_json(var) 
        end
      } unless json_hash['copolymerConnectivity'].nil?
      ...
      result['monomerSet'] = json_hash['monomerSet'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          SubstancePolymerMonomerSet.transform_json(var) 
        end
      } unless json_hash['monomerSet'].nil?
      result['repeat'] = json_hash['repeat'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          SubstancePolymerRepeat.transform_json(var) 
        end
      } unless json_hash['repeat'].nil?
