module FHIR
  ...
  class InsurancePlanPlan < BackboneElement
    ...
    def self.transform_json(json_hash, target = InsurancePlanPlan.new)
      ...
      result['generalCost'] = json_hash['generalCost'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          InsurancePlanPlanGeneralCost.transform_json(var) 
        end
      } unless json_hash['generalCost'].nil?
      result['specificCost'] = json_hash['specificCost'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          InsurancePlanPlanSpecificCost.transform_json(var) 
        end
      } unless json_hash['specificCost'].nil?
