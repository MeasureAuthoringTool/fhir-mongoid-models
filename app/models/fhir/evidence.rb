module FHIR
  ...
  class Evidence < DomainResource
    ...
    embeds_one :exposureBackground, class_name: 'FHIR::Reference'    
    embeds_many :exposureVariant, class_name: 'FHIR::Reference'    
    embeds_many :outcome, class_name: 'FHIR::Reference'    
    ...
    def as_json(*args)
      ...
      unless self.exposureBackground.nil? 
        result['exposureBackground'] = self.exposureBackground.as_json(*args)
      end
      unless self.exposureVariant.nil?  || !self.exposureVariant.any? 
        result['exposureVariant'] = self.exposureVariant.map{ |x| x.as_json(*args) }
      end
      unless self.outcome.nil?  || !self.outcome.any? 
        result['outcome'] = self.outcome.map{ |x| x.as_json(*args) }
      end
      result.delete('id')
    ...
    def self.transform_json(json_hash, target = Evidence.new)
      ...
      result['exposureVariant'] = json_hash['exposureVariant'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          Reference.transform_json(var) 
        end
      } unless json_hash['exposureVariant'].nil?
      result['outcome'] = json_hash['outcome'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          Reference.transform_json(var) 
        end
      } unless json_hash['outcome'].nil?
