    ...
    def self.transform_json(json_hash, target = ContractTerm.new)
      ...
      result['securityLabel'] = json_hash['securityLabel'].map { |var| 
        unless var['resourceType'].nil?
          ...
          ContractTermSecurityLabel.transform_json(var) 
        end
      ...
      result['asset'] = json_hash['asset'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          ContractTermAsset.transform_json(var) 
        end
      } unless json_hash['asset'].nil?
      result['action'] = json_hash['action'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          ContractTermAction.transform_json(var) 
        end
