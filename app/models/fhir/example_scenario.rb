module FHIR
  ...
  class ExampleScenario < DomainResource
    ...
    embeds_many :actor, class_name: 'FHIR::ExampleScenarioActor'    
    embeds_many :instance, class_name: 'FHIR::ExampleScenarioInstance'    
    embeds_many :process, class_name: 'FHIR::ExampleScenarioProcess'    
    embeds_many :workflow, class_name: 'FHIR::PrimitiveCanonical'    
    ...
    def as_json(*args)
      ...
      unless self.workflow.nil?  || !self.workflow.any? 
        result['workflow'] = self.workflow.compact().map{ |x| x.value }
        serialized = Extension.serializePrimitiveExtensionArray(self.workflow)                              
        result['_workflow'] = serialized unless serialized.nil? || !serialized.any?
      end
      result.delete('id')
    ...
    def self.transform_json(json_hash, target = ExampleScenario.new)
      ...
      result['actor'] = json_hash['actor'].map { |var| 
        unless var['resourceType'].nil?
          ...
          ExampleScenarioActor.transform_json(var) 
        end
      ...
      result['instance'] = json_hash['instance'].map { |var| 
        unless var['resourceType'].nil?
          ...
          ExampleScenarioInstance.transform_json(var) 
        end
