module FHIR
  ...
  class CareTeam < DomainResource
    ...
    def as_json(*args)
      ...
      unless self.managingOrganization.nil?  || !self.managingOrganization.any? 
        result['managingOrganization'] = self.managingOrganization.map{ |x| x.as_json(*args) }
      end
    ...
    def self.transform_json(json_hash, target = CareTeam.new)
      ...
      result['status'] = CareTeamStatus.transform_json(json_hash['status'], json_hash['_status']) unless json_hash['status'].nil?
