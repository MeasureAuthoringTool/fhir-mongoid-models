    ...
    def self.transform_json(json_hash, target = InsurancePlan.new)
      ...
      result['contact'] = json_hash['contact'].map { |var| 
        unless var['resourceType'].nil?
          ...
          InsurancePlanContact.transform_json(var) 
        end
      ...
      result['coverage'] = json_hash['coverage'].map { |var| 
        unless var['resourceType'].nil?
          ...
          InsurancePlanCoverage.transform_json(var) 
        end
      } unless json_hash['coverage'].nil?
      result['plan'] = json_hash['plan'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          InsurancePlanPlan.transform_json(var) 
        end
      } unless json_hash['plan'].nil?
