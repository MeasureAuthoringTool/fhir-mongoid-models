module FHIR
  ...
  class DiagnosticReport < DomainResource
    ...
    def self.transform_json(json_hash, target = DiagnosticReport.new)
      ...
      result['resultsInterpreter'] = json_hash['resultsInterpreter'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          Reference.transform_json(var) 
        end
      } unless json_hash['resultsInterpreter'].nil?
      ...
      result['result'] = json_hash['result'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          Reference.transform_json(var) 
        end
      } unless json_hash['result'].nil?
      result['imagingStudy'] = json_hash['imagingStudy'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          Reference.transform_json(var) 
        end
      } unless json_hash['imagingStudy'].nil?
      result['media'] = json_hash['media'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          DiagnosticReportMedia.transform_json(var) 
        end
      } unless json_hash['media'].nil?
      ...
      result['conclusionCode'] = json_hash['conclusionCode'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          CodeableConcept.transform_json(var) 
        end
      } unless json_hash['conclusionCode'].nil?
      result['presentedForm'] = json_hash['presentedForm'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          Attachment.transform_json(var) 
        end
      } unless json_hash['presentedForm'].nil?
