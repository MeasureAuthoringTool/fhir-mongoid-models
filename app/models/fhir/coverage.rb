module FHIR
  ...
  class Coverage < DomainResource
    ...
    def self.transform_json(json_hash, target = Coverage.new)
      ...
      result['payor'] = json_hash['payor'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          Reference.transform_json(var) 
        end
      } unless json_hash['payor'].nil?
      result['class'] = json_hash['class'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          CoverageClass.transform_json(var) 
        end
      ...
      result['costToBeneficiary'] = json_hash['costToBeneficiary'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          CoverageCostToBeneficiary.transform_json(var) 
        end
      } unless json_hash['costToBeneficiary'].nil?
      ...
      result['contract'] = json_hash['contract'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          Reference.transform_json(var) 
        end
      } unless json_hash['contract'].nil?
