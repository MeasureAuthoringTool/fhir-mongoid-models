module FHIR
  ...
  class Patient < DomainResource
    ...
    def self.transform_json(json_hash, target = Patient.new)
      ...
      result['photo'] = json_hash['photo'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          Attachment.transform_json(var) 
        end
      } unless json_hash['photo'].nil?
      result['contact'] = json_hash['contact'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          PatientContact.transform_json(var) 
        end
      ...
      result['communication'] = json_hash['communication'].map { |var| 
        unless var['resourceType'].nil?
          ...
          PatientCommunication.transform_json(var) 
        end
      } unless json_hash['communication'].nil?
      result['generalPractitioner'] = json_hash['generalPractitioner'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          Reference.transform_json(var) 
        end
      } unless json_hash['generalPractitioner'].nil?
      ...
      result['link'] = json_hash['link'].map { |var| 
        unless var['resourceType'].nil?
          ...
          PatientLink.transform_json(var) 
        end
