module FHIR
  ...
  class ClinicalImpression < DomainResource
    ...
    def self.transform_json(json_hash, target = ClinicalImpression.new)
      ...
      result['problem'] = json_hash['problem'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          Reference.transform_json(var) 
        end
      } unless json_hash['problem'].nil?
      result['investigation'] = json_hash['investigation'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          ClinicalImpressionInvestigation.transform_json(var) 
        end
      } unless json_hash['investigation'].nil?
      ...
      result['finding'] = json_hash['finding'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          ClinicalImpressionFinding.transform_json(var) 
        end
      } unless json_hash['finding'].nil?
      result['prognosisCodeableConcept'] = json_hash['prognosisCodeableConcept'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          CodeableConcept.transform_json(var) 
        end
      } unless json_hash['prognosisCodeableConcept'].nil?
      result['prognosisReference'] = json_hash['prognosisReference'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          Reference.transform_json(var) 
        end
      } unless json_hash['prognosisReference'].nil?
