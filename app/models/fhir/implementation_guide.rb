    ...
    def self.transform_json(json_hash, target = ImplementationGuide.new)
      ...
      result['dependsOn'] = json_hash['dependsOn'].map { |var| 
        unless var['resourceType'].nil?
          ...
          ImplementationGuideDependsOn.transform_json(var) 
        end
      } unless json_hash['dependsOn'].nil?
      result['global'] = json_hash['global'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          ImplementationGuideGlobal.transform_json(var) 
        end
      } unless json_hash['global'].nil?
