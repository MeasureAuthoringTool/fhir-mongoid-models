module FHIR
  ...
  class MedicationDispense < DomainResource
    ...
    def self.transform_json(json_hash, target = MedicationDispense.new)
      ...
      result['supportingInformation'] = json_hash['supportingInformation'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          Reference.transform_json(var) 
        end
      } unless json_hash['supportingInformation'].nil?
      result['performer'] = json_hash['performer'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          MedicationDispensePerformer.transform_json(var) 
        end
      } unless json_hash['performer'].nil?
      result['location'] = Reference.transform_json(json_hash['location']) unless json_hash['location'].nil?
      result['authorizingPrescription'] = json_hash['authorizingPrescription'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          Reference.transform_json(var) 
        end
      } unless json_hash['authorizingPrescription'].nil?
      ...
      result['receiver'] = json_hash['receiver'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          Reference.transform_json(var) 
        end
      } unless json_hash['receiver'].nil?
      ...
      result['dosageInstruction'] = json_hash['dosageInstruction'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          Dosage.transform_json(var) 
        end
      } unless json_hash['dosageInstruction'].nil?
      ...
      result['detectedIssue'] = json_hash['detectedIssue'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          Reference.transform_json(var) 
        end
      } unless json_hash['detectedIssue'].nil?
      result['eventHistory'] = json_hash['eventHistory'].map { |var| 
        unless var['resourceType'].nil?
          Object.const_get('FHIR::' + var['resourceType']).transform_json(var)
        else
          Reference.transform_json(var) 
        end
      } unless json_hash['eventHistory'].nil?
